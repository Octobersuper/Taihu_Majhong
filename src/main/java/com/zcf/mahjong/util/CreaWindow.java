...
public class CreaWindow {
    ...
    *//**
    ...
    public void CreateWindow(){
        ...
        String[] values = new String[]{"yiwan","erwan","sanwan","siwan","wuwan","liuwan","qiwan","bawan","jiuwan","yibing","erbing","sanbing","sibing","wubing","liubing","qibing","babing","jiubing","yitiao","ertiao","santiao","sitiao","wutiao","liutiao","qitiao","batiao","jiutiao","dongfeng","nanfeng","xifeng","beifeng","hongzhong","facai","baipi"};
        //调牌值
        		...
        		if(onwer!=null&&!"".equals(onwer)){
        			...
        			for(int i=0;i<pkbean.getBrands().size();i++){
        				...
        				if(index==Integer.parseInt(key)){
        					pkbean.setOnkey(i);
            				textArea.setText(textArea.getText()+Mahjong_Util.mahjong_Util.getBrand_str(index)+"已调                                                \n");
            				break;
        				}
        				if(i+1==pkbean.getBrands().size()){
            				textArea.setText(textArea.getText()+"没有此牌                                               \n");
            			}
        			}
        		}else{
    	...
    	button.addActionListener(new ActionListener() {
             public void actionPerformed(ActionEvent e) {
            	 ...
            	 for(String key:pkmap.keySet()){
	            	 ...
	            	 String nickname = pkbean.getBanker()==0?"未选庄":pkbean.getUserBean(pkbean.getBanker()).getNickname();
	            	 pageMap.put("banker", nickname);
	            	 pageMap.put("gamenumber", pkbean.getGame_number()); 
	            	 pageMap.put("maxnumber", pkbean.getMax_number());
	            	 pageMap.put("draw_s", Mahjong_Util.mahjong_Util.getBrand_str(pkbean.getDraw_s()));
	            	 pageMap.put("max_person", pkbean.getMax_person()+"人房");
	            	 String state = "";
	            	 if(pkbean.getState()==0)state="等待加入";
	            	 if(pkbean.getState()==1)state="准备阶段";
	            	 if(pkbean.getState()==2)state="已开始";
	            	 if(pkbean.getState()==3)state="正在结算";
	            	 if(pkbean.getState()==4)state="已结算";
