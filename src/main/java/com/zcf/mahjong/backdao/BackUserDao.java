package com.zcf.mahjong.backdao;
...
public class BackUserDao {
     ...
     * @return
     */
    public Object onThirteen(int backuserid, int backuserids) {
        // 先判断是否拥有超管权限---有超管才可以赋权超管
        int i = getRole(backuserid);
        if (i != 1) {
            return 110;// 无超管权限
        ...
        String sql = "update game_system SET thirteen=1 WHERE backuserid=?";
        String string = baseDao.executeUpdate(sql, new Object[]{backuserids});
        if ("success".equals(string)) {
            return 0;
        }
     ...
     * @return
     */
    public Object offThirteen(int backuserid, int backuserids) {
        // 先判断是否拥有超管权限---有超管才可以赋权超管
        int i = getRole(backuserid);
        if (i != 1) {
            return 110;// 无超管权限
        ...
        String sql = "update game_system SET role=0,thirteen=0 WHERE backuserid=?";
        String string = baseDao.executeUpdate(sql, new Object[]{backuserids});
        if ("success".equals(string)) {
            return 0;
        }
     ...
     * @return
     */
    public Object onFourteen(int backuserid, int backuserids) {
        // 先判断是否拥有超管权限---有超管才可以赋权超管
        int i = getRole(backuserid);
        if (i != 1) {
            return 110;// 无超管权限
        ...
        String sql = "update game_system SET fourteen=1 WHERE backuserid=?";
        String string = baseDao.executeUpdate(sql, new Object[]{backuserids});
        if ("success".equals(string)) {
            return 0;
        }
     ...
     * @return
     */
    public Object offFourteen(int backuserid, int backuserids) {
        // 先判断是否拥有超管权限---有超管才可以赋权超管
        int i = getRole(backuserid);
        if (i != 1) {
            return 110;// 无超管权限
        ...
        String sql = "update game_system SET role=0,fourteen=0 WHERE backuserid=?";
        String string = baseDao.executeUpdate(sql, new Object[]{backuserids});
        if ("success".equals(string)) {
            return 0;
        }
     ...
     * @return
     */
    public Object onFifteen(int backuserid, int backuserids) {
        // 先判断是否拥有超管权限---有超管才可以赋权超管
        int i = getRole(backuserid);
        if (i != 1) {
            return 110;// 无超管权限
        ...
        String sql = "update game_system SET fifteen=1 WHERE backuserid=?";
        String string = baseDao.executeUpdate(sql, new Object[]{backuserids});
        if ("success".equals(string)) {
            return 0;
        }
     ...
     * @return
     */
    public Object offFifteen(int backuserid, int backuserids) {
        // 先判断是否拥有超管权限---有超管才可以赋权超管
        int i = getRole(backuserid);
        if (i != 1) {
            return 110;// 无超管权限
        ...
        String sql = "update game_system SET role=0,fifteen=0 WHERE backuserid=?";
        String string = baseDao.executeUpdate(sql, new Object[]{backuserids});
        if ("success".equals(string)) {
            return 0;
        }
    ...
    public Map<String, Object> getuser(int pagenum, String backuserid, Map<String, Object> rmap) {
        ...
        if (rmap.get("userid") != null) {
            ...
            String[] ss = new String[]{"10000", "1000", "100", "10", "1"};
            for (int i = 0; i < ss.length; i++) {
                int indexs = rmap.get("userid").toString().indexOf(ss[i]);
                if (indexs != -1) {
                    newuser = rmap.get("userid").toString().substring(indexs + ss[i].length(),
    ...
    public Object getrechargerecord(Integer pagenum, String backuserid, String zuserid, String startTime) {
        ...
        if (StringUtils.isEmpty(startTime)) {
            String sql = "select * from game_recharge where userid=? ORDER BY date desc LIMIT ?,10";
            ...
            String sql = "select * from game_recharge where userid=? AND date > ? AND date < ? ORDER BY date desc LIMIT ?,10";
    ...
    public Map<String, Object> getcard(int pagenum, Map<String, Object> rmap, String backuserid) {
        ...
        List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
        // 获取牌友圈的权限
        int menuone = selectMenuSeven(backuserid);
        if (menuone == 0) {
            rmap.put("state", 101);
            return rmap;
        }
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                List<String> onlinepersons = (List<String>) onlineperson(list.get(i).get("userid").toString());
                personnum = 0;
                for (int j = 0; j < onlinepersons.size(); j++) {
                    if (Public_State.clients.get(onlinepersons.get(j)) != null) {
                        personnum++;
                    }
                }
                // 当前俱乐部人数
                System.out.println(personnum);
                list.get(i).put("personnum", personnum);
            }
            Map<String, Object> map = new HashMap<String, Object>();
    ...
    public Object downdiamond(String userid, String diamond, String zuserid) {
        ...
        if ("success".equals(str)) {
            insertddiamond(diamond, zuserid);
            return "success";
        }
        return -1;
    }
     ...
     * @return
     */
    public Object getActivity(String backuserid) {
        List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
        Map<String, Object> rmap = new HashMap<String, Object>();
        int backuserids = selectFourteen(backuserid);
        if (backuserids == 0) {
            rmap.put("state", 101);// 权限不足
            return rmap;
        }
    ...
    private int selectFourteen(String backuserid) {
        baseDao.executeAll(sql, new Object[]{backuserid});
        try {
            if (baseDao.resultSet.next()) {
                return baseDao.resultSet.getInt("fourteen");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @return
     */
    public Object updateActivity(int activityid, String value) {
        String sql = "update game_activity SET `value`=? WHERE activityid=?";
        return baseDao.executeUpdate(sql, new Object[]{value, activityid});
    }
     ...
     * @return
     */
    public Object getAgency(String backuserid) {
        List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
        Map<String, Object> rmap = new HashMap<String, Object>();
        // 查看消耗兑换权限
        int backuserids = selectFifteen(backuserid);
        if (backuserids == 0) {
            rmap.put("state", 101);// 权限不足
            return rmap;
        }
    ...
    private int selectFifteen(String backuserid) {
        baseDao.executeAll(sql, new Object[]{backuserid});
        try {
            if (baseDao.resultSet.next()) {
                return baseDao.resultSet.getInt("fifteen");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @return
     */
    public Object updateAgency(int agencyid, String value) {
        String sql = "update game_agency SET `value`=? WHERE agencyid=?";
        return baseDao.executeUpdate(sql, new Object[]{value, agencyid});
    }
    ...
    public Object getfreediamond(String backuserid) {
        ...
        Map<String, Object> rmap = new HashMap<String, Object>();
        // 任务权限
        int backuserids = selectMenuSeven(backuserid);
        if (backuserids == 0) {
            rmap.put("state", 101);// 权限不足
            return rmap;
        }
    ...
    public Object getgamerecord(String backuserid, String startTime) {
        ...
        if (StringUtils.isEmpty(startTime)) {
            String sql = "SELECT SUM(money) AS diamond ,(select SUM(money) from game_recharge where type=1) AS money from game_recharge where type=3";
            baseDao.executeAll(sql, new Object[]{});
            try {
                while (baseDao.resultSet.next()) {
                    Map<String, Object> map = UtilClass.utilClass.getSqlMap("/sql.properties", baseDao.resultSet,
            ...
            String startTimes = startTime + " 00:00:00";
            String endTimes = startTime + " 23:59:59";
            String sql = "SELECT SUM(money) AS diamond ,(select SUM(money) from game_recharge where type=1 AND date > ? AND date < ? ) AS money from game_recharge where type=3 AND date > ? AND date < ?";
    ...
    public Object lookCircle(String circlenumber) {
        ArrayList<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
        String sql = "SELECT gu.userid,gu.nickname,gu.diamond,gcu.date FROM user_table AS gu LEFT JOIN game_card_user AS gcu ON gcu.userid = gu.userid LEFT JOIN game_card_circle AS gcc ON gcc.circlenumber = gcu.circlenumber WHERE gcu.circlenumber = ?";
        ...
        try {
            while (baseDao.resultSet.next() && "success".equals(executeAll)) {
                ...
                int userid = Integer.parseInt(map.get("userid").toString());
                if (userid >= 10000 && userid < 100000)
                    map.put("userid", 1 + "" + userid);
                if (userid >= 1000 && userid < 10000)
                    map.put("userid", 10 + "" + userid);
                if (userid >= 100 && userid < 1000)
                    map.put("userid", 100 + "" + userid);
                if (userid >= 10 && userid < 100)
                    map.put("userid", 1000 + "" + userid);
                if (userid < 10)
                    map.put("userid", 10000 + "" + userid);
                list.add(map);
            }
            return list;
        } catch (SQLException e) {
    ...
    public Object author(String userid) {
        String sql = "UPDATE user_table SET fid=1 where userid=?";
    ...
    public Object delauthor(String userid) {
        String sql = "UPDATE user_table SET fid=0 where userid=?";
        return baseDao.executeUpdate(sql, new Object[]{userid});
    }
     ...
     * @return
     */
    private int selectThirteenteen(String backuserid) {
        baseDao.executeAll(sql, new Object[]{backuserid});
        try {
            if (baseDao.resultSet.next()) {
                return baseDao.resultSet.getInt("thirteen");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @return
     */
    public Object updgui(String value, int introduceid) {
        String sql = "UPDATE game_introduce SET value=? where introduceid=?";
        return baseDao.executeUpdate(sql, new Object[]{value, introduceid});
    }
     ...
     * @return
     */
    public Object getBrandsType(String backuserid) {
        ArrayList<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
        String sql = "select * from brands_type";
        String executeAll = baseDao.executeAll(sql, new Object[]{});
        ...
        try {
            while (baseDao.resultSet.next() && "success".equals(executeAll)) {
                ...
                Map<String, Object> rmap = UtilClass.utilClass.getSqlMap("/sql.properties", baseDao.resultSet,
                        "sql_getbrandstype");
                String[] brandtypes = rmap.get("brandtype").toString().split("-");
                for(int i=0;i<brandtypes.length;i++){
                    int num=Integer.parseInt(brandtypes[i]);
                    brandlist.add(num);
                }
                rmap.put("brandtype", transinfo(brandlist));
                list.add(rmap);
            }
            return list;
        } catch (SQLException e) {
