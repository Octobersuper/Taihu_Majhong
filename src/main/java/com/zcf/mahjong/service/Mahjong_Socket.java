...
public class Mahjong_Socket {
	...
	public void onMessage(String message) {
		...
		// 心跳连接
		/*
		 * if ("heartbeat".equals(jsonTo.get("type"))) { returnMap.put("type",
		 * "heartbeat"); sendMessageTo(returnMap); }
		 */
		// 创建房间
			...
			if (roomBean != null
				...
				if (roomBean == null) {
					...
					roomBean.getRoomBean_Custom(
							"roomno-user_positions-gps-max_number", returnMap,
		...
		if ("Matching".equals(jsonTo.get("type"))) {
			...
			if (state == 0) {
				...
				if (roomBean == null) {
					...
					roomBean.getRoomBean_Custom(
							"roomno-max_person-user_positions-max_number-game_type-fen",
							returnMap,
		...
		if ("get_brand".equals(jsonTo.get("type"))) {// 流局
			...
			if (roomBean.getBrands().size() <= roomBean.getGame_userlist()
				...
				// 检测是否可以补杠
				// returnMap.put("is_repair_bar",Mahjong_Util.mahjong_Util.IS_Repair_Bar_Bump(userBean,roomBean.getDraw()));
				returnMap.put("userid", userBean.getUserid());
		...
		if ("out_brand".equals(jsonTo.get("type"))) {
			...
			int value = Mahjong_Util.mahjong_Util.getBrand_Value(outbrand);
			if (value == 27 || value == 61 || value == 95 || value == 31
				...
				returnMap.put("mtype", 0);
			}

			// 出牌并且判定是否有碰或杠
			int state = gameService.OutBrand(roomBean, outbrand, userBean,
					returnMap);
		...
		if ("bump".equals(jsonTo.get("type"))) {
			...
			if (p_userid != 0) {
				...
				gameService.End_Hu(userBean,
						Integer.parseInt(jsonTo.get("brand")), roomBean,
						p_userid);
				// 弃胡//结算500=已经结算 501=等待别人胡牌操作 502=等待别人结算
		...
		if ("endhu".equals(jsonTo.get("type"))) {
			...
			// 0不胡牌 900=4朴一将 890=1癞子3朴1将 880=1癞子4朴 缺将
			int state = gameService.End_Hu(userBean,
					Integer.parseInt(jsonTo.get("brand")), roomBean, p_userid);
			if (state != 0) {
				// 点炮 2分
				userBean.setPower(2);
				// 检测是否是杠后点炮1明杠后点炮2暗杠后点炮3补杠后点炮
				roomBean.IS_Bar_Userid(p_userid);
				// 结算检测
				MahjongUtils mahjongUtils = new MahjongUtils(roomBean,
						userBean, Integer.parseInt(jsonTo.get("brand")));
				mahjongUtils.getBrandKe(roomBean, userBean,
						Integer.parseInt(jsonTo.get("brand")), 1);
		...
		if ("endhu_this".equals(jsonTo.get("type"))) {
			...
			// 自摸
			mahjongUtils.getBrandKe(roomBean, userBean,
					Integer.parseInt(jsonTo.get("brand")), 2);
			// 自摸结算
		...
		if ("end_wifi".equals(jsonTo.get("type"))) {
			...
			if (roomBean == null) {
				...
				// 查询出房间信息返回
				roomBean.getRoomBean_Custom(
						"cannon-dice-exit_game-user_log-end_userid-roomno-user_positions-brands_count-draw_s-draw-fen-fan-money-gps-banker-game_number-max_number-max_person-state-end_userid-game_type",
						returnMap,
						"userid-nickname-avatarurl-brands-eat_brands-bump_brands-show_brands-hide_brands-out_brands-ip-log_lat-number-is_hustate-hu_state-exit_game");
				// //关闭托管
		...
		if ("get_position".equals(jsonTo.get("type"))) {
			...
			if (roomBean.getGame_userlist().size() == 2) {
				...
				String re = roomBean.getGame_userlist().get(0).getUserid()
						+ "-" + roomBean.getGame_userlist().get(1).getUserid()
		...
		if ("end".equals(jsonTo.get("type"))) {
			...
			roomBean.getUserBean(Integer.parseInt(jsonTo.get("p_userid")))
					.getUser_Custom("userid-number-dqnumber", returnMap);
			returnMap.put("type", "endhu");
			sendMessageTo(returnMap);
			sendMessageTo(returnMap, roomBean);
			roomBean.addLog(returnMap);
